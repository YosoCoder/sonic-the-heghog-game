...
function setup() {
    createCanvas(displayWidth - 30, displayHeight - 145);
  ...
  invisibleGround = createSprite(200,displayHeight - 145,400,10);
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    if((touches.length > 0 || keyDown("space"))&& sonic.y >= displayHeight / 2) {
      sonic.velocityY = -12;
        jumpSound.play();
        touches = [];
...
function spawnObstacles(){
 if (frameCount % 150 === 0){
   var obstacle = createSprite(displayWidth,displayHeight - 175,10,40);
